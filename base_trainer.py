...
class BaseTrainer:
    ...
    def train(self, max_epochs, start_epoch=0, load_latest=False, fail_safe=True, load_previous_ckpt=False, distill=False):
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(start_epoch + 1, max_epochs + 1):  # <-- Use start_epoch here
                    # ----------------------------------------------------
                    if self.settings.base_seed is not None:
                        # Set the same fixed seed every epoch start
                        init_seeds(self.settings.base_seed)
                        # Optional: Print to confirm it's running each epoch
                        if self.settings.local_rank in [-1, 0]:  # Print only on main process
                            print(f"==> Epoch {epoch}: Seed set to {self.settings.base_seed}")
                    # ----------------------------------------------------
                    self.epoch = epoch
    ...
    def save_checkpoint(self):
        ...
        state = {
            ...
            'lr_scheduler': self.lr_scheduler.state_dict(),  # <-- ADD THIS
            'rng_state': torch.get_rng_state(),              # <-- ADD THIS
            'rng_state_cuda': torch.cuda.get_rng_state(),    # <-- ADD THIS
            'stats': self.stats,
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        for key in fields:
            ...
            # ADD THESE NEW 'elif' BLOCKS
            # ----------------------------------------------------
            elif key == 'lr_scheduler':
                self.lr_scheduler.load_state_dict(checkpoint_dict[key])
            elif key == 'rng_state':
                torch.set_rng_state(checkpoint_dict[key])
            elif key == 'rng_state_cuda':
                torch.cuda.set_rng_state(checkpoint_dict[key])
            # ----------------------------------------------------
            else:
