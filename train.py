...
def parse_args():
    ...
    parser.add_argument('--resume', type=int, default=0, help='Resume from latest checkpoint.')  # <-- ADD THIS LINE
...
def main():
    args = parse_args()
    if args.mode == "single":
        train_cmd = "python lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d --resume %d --resume_epoch %s" \
                    % (args.script, args.config, args.save_dir, args.use_lmdb, args.resume, args.resume_epoch)
    elif args.mode == "multiple":
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --resume %d --resume_epoch %s" \
                    % (args.nproc_per_node, args.script, args.config, args.save_dir, args.use_lmdb, args.resume, args.resume_epoch)
    else:
